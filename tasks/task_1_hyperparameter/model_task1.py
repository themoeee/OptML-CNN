...
class SimpleCNN(nn.Module):
    def __init__(self, kernel_size):
        ...
        padding = kernel_size // 2                                  # To maintain spatial dimensions
                                                                    # This part is needed for optuna search, otherwise changing kernel size would clash with padding

        self.features = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size, padding=padding),         #padding needed to keep image size the same
            nn.ReLU(),
            nn.MaxPool2d(2),
